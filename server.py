...
# 1. Create Gradio demo interface
with gr.Blocks(title="Daily English AI Tutor", fill_width=True) as demo:
    gr.HTML('''
        <div style="text-align: center; margin-bottom: 12px; font-family: system-ui, sans-serif;">
            ...
            <a href="/app" target="_blank" style="display:inline-block; padding:10px 22px; background:linear-gradient(135deg, #4f46e5, #7c3aed); color:white; border-radius:8px; text-decoration:none; font-weight:600; font-size:15px; box-shadow:0 4px 12px rgba(79,70,229,0.3);">
                🚀 Mở Ứng Dụng Toàn Màn Hình (Khuyên dùng)
            </a>
        </div>
        <iframe src="/app" style="width: 100%; height: 850px; border: 1px solid #e2e8f0; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.06);"></iframe>
    ''')
...
# 2. Create the Gradio FastAPI application
app = gr.routes.App.create_app(demo)

# 3. Mount our static files and full FastAPI app routes
